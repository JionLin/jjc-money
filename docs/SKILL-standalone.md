...
> ⚠️ 以下为历史快照（2026-04），估值锚点会随 EPS 增长漂移。请结合实时数据使用。

| 标的 | Forward PE | PEG | 信号 | 加仓甜蜜区 | 减仓触发区 |
|------|:---------:|:---:|:----:|:---------:|:---------:|
| **NVDA** | ~24x | **0.69** | 🟢低估 | ≤$165 | $240+ |
| **MSFT** | ~24x | 0.9-1.6 | 🟡合理 | $355-$380 | $500+ |
| **META** | ~22x | **1.0** | 🟡合理 | ≤$556 | $720+ |
| **GOOGL** | ~23-30x | 1.5-2.5 | 🟠偏贵 | ≤$140 | $226+ |
| **AMZN** | ~31x | 2.8 | 🔴红旗 | — | — |
| **AAPL** | ~28-30x | ~2.0 | 🔴红旗 | $230-$245 | $280+ |
...
- **Primary Tool**: `financial-datasets` MCP (if available) or web search
  - **Purpose**: Retrieve real-time Forward PE, EPS Growth, and Cash Flow metrics.
  - **Instruction**: When a specific US stock ticker is mentioned, ALWAYS fetch fresh data first before applying mental models.

- **Knowledge Base** (if available in current workspace):
  - Search for files matching: `*美股投资实战*`, `*仓位管理*`, `*deep-analysis*`
  - Search keywords: "估值", "锚点", "加仓", "减仓", "负成本", "VIX"
  - If no local knowledge base exists, rely on the embedded parameters in this SKILL file.



## 7 Execution Protocol (SOP) — v2.0-standalone

Whenever the user asks for a stock analysis, strictly follow these steps:

```
Step 1 — [Data Fetching]
  → 获取 Forward PE / EPS Growth / FCF（MCP / 网络搜索 / 用户提供）

Step 2 — [Historical Context]
  → 如当前工作区有知识库文件，搜索该标的的历史提及
  → 对比本文件内嵌的估值校准表（§2.1 Appendix）中的历史买卖区间
  → 如无本地知识库，直接使用本文件内嵌参数
...
Step 4 — [Consistency Check]
  → 对比当前估值 vs 内嵌校准表历史锚点（Forward PE / PEG / 买卖区间）
  → 如果有冲突，解释市场条件变化
...
| v2.0 | 2026-04-25 | **反哺升级**：注入实战案例、估值校准表、新增§2.5赛道+§2.6情景、三账户+金字塔、踩坑案例、SOP 6步 |
| v2.0-standalone | 2026-04-25 | **独立版**：去除所有本地文件硬编码引用，§6-§7改为自适应通用写法，可在任意项目中独立运行 |
